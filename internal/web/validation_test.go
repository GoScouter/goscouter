...
func TestIsValidSite(t *testing.T) {
	tests := []struct {
		...
		{"2xx is valid", http.StatusOK, true},
		{"2xx upper bound", http.StatusIMUsed, true}, // 226
		{"3xx is invalid", http.StatusMovedPermanently, false},
		{"4xx is invalid", http.StatusNotFound, false},
		{"5xx is invalid", http.StatusInternalServerError, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := IsValidSite(srv.URL); got != tt.want {
				t.Fatalf("IsValidSite = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestIsValidSiteUnreachable(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
	url := srv.URL
	srv.Close()

	if IsValidSite(url) {
		t.Fatal("expected IsValidSite to be false for unreachable host")
	}
}
